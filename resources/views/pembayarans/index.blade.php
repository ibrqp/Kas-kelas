...
<body style="background: lightgray">
                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($pembayarans as $key => $post)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $post->siswa->nama }}</td>
                                        <td>{{ $post->tgl_bayar }}</td>
                                        <td>{{ $total[$post->siswa->id] }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('pembayaran.detail', $post->id_siswa) }}"
                                                class="btn btn-sm btn-primary">
                                                DETAIL
                                            </a>
                                            {{-- <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#editE" data-siswa-id="{{ $post->siswa->id }}">
                                                DETAIL
                                            </a> --}}
                                        </td>
                                    </tr>
    ...
    <script>
        ...
        function updateTotal(input) {
            ...
            // Set the formatted value in the display element
            document.getElementById('formattedJumlahBayar').innerText = formattedValue;
        }
